 ...
 */
uint8_t AddInput(const input_s *input,inputState_e currentIpState)
{
    if(numInputs < NUM_TOTAL_INPUTS)
    {
        inputHdl[numInputs].input = input;
        inputHdl[numInputs].filteredInput = INPUT_ON;
        inputHdl[numInputs].currentState = currentIpState;
