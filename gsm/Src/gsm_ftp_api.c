...
char ftpCmdBuff[FTP_CMD_BUFF_SIZE];
// ufs free size
int ufsFreeSize = 0;
// file downloaded size
uint32_t ftpDownloadedFileSize = 0;
int32_t getFileSize = 0;
uint8_t flagFtpDecodeUrl = 0;
// ftp file struct holding all the file details
ftpFile_st ftpFile;

extern int gsmFileHandle;

#define LOCAL_UFS_FILENAME      "UFS:fotafile.bin"

FtpDownloadStatusCbFnPtr FtpStatusCb = NULL;
FTPCmdReply_et ftpRetCode = FTP_UNKNOWN_ERROR;
FTPCmdReply_et ftpSmsRetCode = FTP_UNKNOWN_ERROR;
...
gsmFtpCmd_st ftpCmdList[]= {
    ...
    {"AT+QFTPCFG=\"contextid\",1\r",GSM_FTP_DEFAULT_CMD_TIMEOUT,FtpOkRespCb},
    {"AT+QFTPCFG=\"account\",\"",GSM_FTP_DEFAULT_CMD_TIMEOUT,FtpOkRespCb},
    {"AT+QFTPCFG=\"filetype\",0\r",GSM_FTP_DEFAULT_CMD_TIMEOUT,FtpOkRespCb},
    {"AT+QFTPCFG=\"transmode\",1\r",GSM_FTP_DEFAULT_CMD_TIMEOUT,FtpOkRespCb},
    {"AT+QFTPCFG=\"rsptimeout\",120\r",GSM_FTP_DEFAULT_CMD_TIMEOUT,FtpOkRespCb},
    {"AT+QFTPOPEN=",GSM_FTP_OPEN_CMD_TIMEOUT,FtpProcessRespCb},
    {"AT+QFTPCWD=",GSM_FTP_CMD_TIMEOUT,FtpProcessRespCb},
...
{
    return (FTPCmdReply_et)ftpSmsRetCode;
}

void FtpGetErrorString(FTPCmdReply_et errVal)
{
    ftpSmsRetCode = (FTPCmdReply_et)ftpRetCode;
    switch(errVal)
    {
        case 0:
...
gsmRespCb_et FtpProcessRespCb(gsmCmdResponseCode_et gsmResp, uint8_t *lineBuff, uint16_t len)
{
    ...
    switch(gsmResp)
    {
        ...
        case RESP_FTP_CONFIG:
            // "+QFTPCFG:" - offset is 9
            ftpRetCode = (FTPCmdReply_et)atoi((const char *)(lineBuff + 9));
        ...
        case RESP_FTP_OPEN:
            // "+QFTPOPEN: 0,0" offset is 10
            ftpRetCode = (FTPCmdReply_et)atoi((const char *)(lineBuff + 10));
            if(ftpRetCode == FTP_SUCCESS)
            {
                gsmRespRetVal = GSM_RESP_CB_OK_COMPLETE;
                ftpAtCmdState = AT_CMD_SUCCESS;
        ...
        case RESP_FTP_SIZE:
            ...
            if(ftpRetCode == FTP_SUCCESS)
            {
                ...
                q = strchr((const char*)lineBuff, '\0');
                if(p && q)
                {
                    memset(valBuff,0,sizeof(valBuff));
                    strncpy(valBuff, p+1, q-p-1);
                    getFileSize = atoi((const char *)valBuff);
                }
            }
            if(getFileSize < 0)
            {
              ftpRetCode = (FTPCmdReply_et)getFileSize;
            }
            if(getFileSize >= 0)
            {
                ftpFile.fileSize = (uint32_t)getFileSize;
                LOG_DBG(CH_GSM,"Get File Size from +QFTPSIZE: : %d",ftpFile.fileSize);
                gsmRespRetVal = GSM_RESP_CB_OK_COMPLETE;
                ftpAtCmdState = AT_CMD_SUCCESS;
            }
            else
        ...
        case RESP_FTP_GETFILE:
            ...
            //getFileSize = atoi((const char *)(lineBuff + 9));
            if(getFileSize < 0)
            {
              ftpRetCode = (FTPCmdReply_et)getFileSize;
            }
            if(getFileSize >= 0)
            {
                ftpDownloadedFileSize = getFileSize;
                LOG_DBG(CH_GSM,"Downloaded File Size from +QFTPGET: : %d",ftpDownloadedFileSize);
        ...
        case RESP_FTP_CLOSE:
            ...
            ftpRetCode = (FTPCmdReply_et)atoi((const char *)(lineBuff + 11));
            if(ftpRetCode == FTP_SUCCESS)
            {
                gsmRespRetVal = GSM_RESP_CB_OK_COMPLETE;
                ftpAtCmdState = AT_CMD_SUCCESS;
...
{
    if(GsmStateIsIdle())// && GsmGprsIsActive())
    {
        if(FtpUrlDecode(urlString,&ftpFile))
        {
            ...
            getFileSize = 0;
            ftpSmsRetCode = FTP_UNKNOWN_ERROR;
...
char* FtpGetCurrentCmd(void)
{
    char* cmd = ftpCmdList[ftpCurCmd].cmd;
    ftpRetCode = FTP_UNKNOWN_ERROR;
    switch(ftpCurCmd)
    {
        ...
        case SET_FTP_CONTEXTID:
        break;
                                     
        case SET_FTP_ACCOUNT:       //AT+QFTPCFG=\"account\",\username\",\"password\"\r"
          sprintf((char *)ftpCmdBuff, "%s%s\",\"%s\"\r",ftpCmdList[SET_FTP_ACCOUNT].cmd,ftpFile.userName,ftpFile.password);
          cmd = ftpCmdBuff;
        break;
        
        case SET_FTP_FILETYPE:
        break;
        
        case SET_FTP_TRANSMODE:
        break;
        
        case SET_FTP_RESPTIMOUT:
        break;
        ...
        case GET_FTP_FILE:
            getFileSize = 0;
            sprintf((char *)ftpCmdBuff, "%s\"%s\",\"%s\"\r",ftpCmdList[GET_FTP_FILE].cmd,ftpFile.fileName, LOCAL_UFS_FILENAME);
            cmd = ftpCmdBuff;
        break;
...
////                        (ftpCmd_et)ftpCurCmd++;
        ...
        case FTP_DOWNLOAD_SUCCESS:
            ftpState = FTP_WAIT_FOR_START;        //again change the state default state
        break;
        
        case FTP_DOWNLOAD_FAIL:
            ftpState = FTP_WAIT_FOR_START;
        break;
    }
}